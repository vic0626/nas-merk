...
namespace Nas.Services.Authentication.External
{
    public static partial class ExternalAuthorizerHelper
    {
        ...
        {
            var session = GetSession();
            session.Remove("Nas.externalauth.parameters");
        }

        public static void AddErrorsToDisplay(string error)
        {
            var session = GetSession();
            var errors = session["Nas.externalauth.errors"] as IList<string>;
            if (errors == null)
            {
                errors = new List<string>();
                session.Add("Nas.externalauth.errors", errors);
            }
            errors.Add(error);
        }
        ...
        public static IList<string> RetrieveErrorsToDisplay(bool removeOnRetrieval)
        {
            ...
            if (errors != null && removeOnRetrieval)
                session.Remove("Nas.externalauth.errors");
            return errors;
        }
    }
}