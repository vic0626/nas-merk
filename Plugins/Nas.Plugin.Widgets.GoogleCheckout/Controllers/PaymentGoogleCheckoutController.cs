...
using Nas.Plugin.Payments.GoogleCheckout.Models;
...
namespace Nas.Plugin.Payments.GoogleCheckout.Controllers
{
    public class PaymentGoogleCheckoutController : BaseNasPaymentController
    {
        ...
        {
            string fullResourceName = string.Format("Nas.Plugin.Payments.GoogleCheckout.Samples.{0}", resourceName);
        ...
        public ActionResult PaymentInfo()
        {
            ...
            return View("Nas.Plugin.Payments.GoogleCheckout.Views.PaymentGoogleCheckout.PaymentInfo", model);
        }
        ...
        public ActionResult SubmitButton()
        {
            try
            {
                ...
                if (processor == null ||
                    ...
                    throw new NasException("Google Checkout module cannot be loaded");
        ...
        public ActionResult NotificationHandler()
        {
            ...
            if (debugModeEnabled)
            {
                if (_webHelper.QueryString<int>("NasCommerceTestNewOrder1") > 0)
                {
                    xmlData = GetSampleFileContent("sample-neworder.txt");
                }
                if (_webHelper.QueryString<int>("NasCommerceTestNewOrder2") > 0)
                {
                    xmlData = GetSampleFileContent("sample-neworder-noShipment.txt");
                }
                else if (_webHelper.QueryString<int>("NasCommerceTestOrderChange") > 0)
                {
                    xmlData = GetSampleFileContent("sample-orderchangestate.txt");
                }
                else if (_webHelper.QueryString<int>("NasCommerceTestRisk") > 0)
                {
