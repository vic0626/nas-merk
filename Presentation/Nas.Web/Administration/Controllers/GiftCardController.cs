...
{
    [AdminAuthorize]
    public partial class GiftCardController : BaseNasController
    {
        #region Fields
        ...
        public ActionResult NotifyRecipient(GiftCardModel model)
        {
            ...
            try
            {
                if (!CommonHelper.IsValidEmail(giftCard.RecipientEmail))
                    throw new NasException("Recipient email is not valid");
                if (!CommonHelper.IsValidEmail(giftCard.SenderEmail))
                    throw new NasException("Sender email is not valid");
